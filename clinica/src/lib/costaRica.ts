/**
 * División Territorial Administrativa de Costa Rica: 7 provincias, 84 cantones
 * y sus distritos. Alimenta los tres selects encadenados de la dirección en la
 * ficha del paciente (PatientFormModal.vue).
 *
 * Incluye los cantones más recientes: Río Cuarto (Alajuela, 2017), Monteverde
 * (Puntarenas, 2021) y Puerto Jiménez (Puntarenas, 2022), ya segregados de
 * Grecia, Puntarenas y Golfito respectivamente.
 *
 * En `patients` se guardan los NOMBRES, no códigos: la ficha queda legible en
 * el admin de Directus y en cualquier export, y un dato viejo sobrevive a un
 * cambio de esta lista. Por eso los selects del formulario agregan siempre el
 * valor guardado aunque ya no figure acá (ver `withCurrent`), en vez de
 * borrarlo en silencio al abrir la ficha.
 */

export const COSTA_RICA: Record<string, Record<string, string[]>> = {
  "San José": {
...
export function provincias(): string[] {
  return Object.keys(COSTA_RICA);
}

export function cantones(provincia: string | null | undefined): string[] {
  if (!provincia) return [];
  return Object.keys(COSTA_RICA[provincia] ?? {});
}

export function distritos(provincia: string | null | undefined, canton: string | null | undefined): string[] {
  if (!provincia || !canton) return [];
  return COSTA_RICA[provincia]?.[canton] ?? [];
}

/**
 * Lista de opciones que incluye el valor ya guardado aunque no esté en el
 * catálogo (fichas viejas, un cantón que cambió de nombre, un dato cargado a
 * mano en Directus). Sin esto el `<select>` no podría representar la ficha y
 * el valor se perdería al guardar.
 */
